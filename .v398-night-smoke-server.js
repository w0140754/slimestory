...
const WORLD_CLOCK_START_GAME_MINUTES = 20 * 60;
...
const NIGHT_SLIME_SPAWN_INTERVAL_SECONDS = 0.7;
...
function handleStructureDestroyRequest(playerId, socket, message) {
  ...
  if (structure.kind === "dugDirt") {
    ...
    const removedDirt = removeAnyStructure(structureId, structure.mapId);
    if (!removedDirt) return;
    broadcastToMap(removedDirt.mapId, { type: "structureRemoved", structureId: removedDirt.id, mapId: removedDirt.mapId, reason: "minedDirt" });
    spawnSharedResource(removedDirt.mapId, "inventoryItem", removedDirt.x, removedDirt.y, {
      itemToken: "resource:dirt",
      itemCount: 1,
      life: 300
    });
    sendJson(socket, { type: "structureDestroyResult", success: true, structureId: removedDirt.id, kind: "dugDirt" });
    return;
  }
...
function handleGroundDigAction(playerId, playerState, payload) {
  ...
  const breakthrough = TERRAIN_RULES.circleCanOccupy(
    ...
    4,
    { allowWater: false }
  ) === true && !playerSpawnPointBlocked(undergroundMapId, x, y, 4);
