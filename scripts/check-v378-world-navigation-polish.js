...
assert.strictEqual(pkg.version, "0.6.11.468", "package version must be current cleanup build");
assert(server.includes('const BUILD_VERSION = "6-11-468";'), "server build marker must be current cleanup build");
assert(read("public", "client-config.js").includes('const CLIENT_BUILD_VERSION = "6-11-468";'), "client build marker must be current cleanup build");
