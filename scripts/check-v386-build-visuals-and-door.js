...
assert(read("public", "client-config.js").includes('const CLIENT_BUILD_VERSION = "6-11-391";'));

assert(game.includes('const BUILD_HOTBAR_ITEMS = Object.freeze(["woodFloor", "woodWall", "woodDoor"]);'), "Wood Door must join the unified build hotbar list");
...
assert(game.includes("alpha * 0.58"), "wall fade should match the canopy-style partial transparency");
...
assert(server.includes("if (serverPointHitsStructureWall(mapId, x, y, Math.max(4, padding))) return false;"), "enemy/world occupancy should continue treating doors as closed");
assert(server.includes('if (!["wood", "stone", "flower", "goldSlimeBubble", "icedCoffee", "woodFloor", "woodWall", "woodDoor"].includes(kind))'), "Wood Door must be valid shared reclaim loot");
