...
assert(game.includes("const LOCAL_FOREGROUND_OCCLUDER_ALPHA = 0.24;"), "universal foreground occlusion alpha missing");
assert(game.includes("function localForegroundOcclusionAlpha(worldRect, drawSortY, alpha = 1)"), "universal foreground occlusion helper missing");
...
assert(server.includes('if (!["wood", "stone", "flower", "goldSlimeBubble", "greenJellyCube", "icedCoffee", "woodFloor", "stoneFloor", "woodWall", "stoneWall", "stoneCube", "caveDoor", "woodDoor", "torch", "chest", "craftingTable", "inventoryItem"].includes(kind))'), "Wood Door must be valid shared reclaim loot");
assert(server.includes('} else if (resource.kind === "woodDoor") {'), "Wood Door pickup restoration missing");
assert(network.includes('["woodWall", "stoneWall", "woodDoor", "caveDoor"].includes(kind)'), "network edge payload must support doors without treating Stone Cube as a boundary");
