...
assert(server.includes('const BUILD_VERSION = "6-11-353";'), 'server build must be v347');
assert(config.includes('const CLIENT_BUILD_VERSION = "6-11-353";'), 'client build must be v347');
assert(html.includes('/shared/combat-balance.js?v=353'), 'combat balance cache key must be v27');
assert(html.includes('/client-enemies.js?v=353') && html.includes('/game.js?v=353'), 'v347 client cache keys missing');
