...
assert(server.includes('const BUILD_VERSION = "6-11-424";'), 'server build must be v347');
assert(config.includes('const CLIENT_BUILD_VERSION = "6-11-424";'), 'client build must be v347');
assert(html.includes('/shared/combat-balance.js?v=424'), 'combat balance cache key must be v27');
assert(html.includes('/client-enemies.js?v=424') && html.includes('/game.js?v=424'), 'v347 client cache keys missing');
