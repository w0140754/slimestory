...
assert.strictEqual(pkg.version, "0.6.11.424", "package version must be v380");
assert(server.includes('const BUILD_VERSION = "6-11-424";'), "server build marker must be v380");
assert(read("public", "client-config.js").includes('const CLIENT_BUILD_VERSION = "6-11-424";'), "client build marker must be v380");
...
assert(server.includes('["wood", "stone", "flower", "goldSlimeBubble", "greenJellyCube", "icedCoffee", "woodFloor", "stoneFloor", "woodWall", "woodDoor", "torch", "chest", "craftingTable"]'), "shared loot must accept all current building piece types");
...
assert(clientWorld.includes('treeTrunkImage.src = "assets/interactive_tree_trunk_damaged_v376.png?v=424";'), "fresh tree must use the clean swapped trunk state");
assert(clientWorld.includes('treeDamagedTrunkImage.src = "assets/interactive_tree_trunk_v376.png?v=424";'), "first-hit tree must use the cut swapped trunk state");

console.log("v380 building/navigation refinement OK: visible floors, 1-0 assignment, Pickaxe reclaim, seam continuity, single-player transition render, and corrected tree states.");
