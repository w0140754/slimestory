...
assert.strictEqual(pkg.version, "0.6.11.407", "package version must be v380");
assert(server.includes('const BUILD_VERSION = "6-11-407";'), "server build marker must be v380");
assert(read("public", "client-config.js").includes('const CLIENT_BUILD_VERSION = "6-11-407";'), "client build marker must be v380");
...
assert(game.includes('const BUILD_HOTBAR_ITEMS = Object.freeze(["woodFloor", "woodWall", "woodDoor", "torch"]);'), "Wood Floor + Wood Wall must be assignable to the 1-9 belt");
...
assert(game.includes('ctx.drawImage(woodFloorStructureImage, x, y, 16, 16);'), "Wood Floor must render the authored 16x16 structure sprite");
...
assert(server.includes('["wood", "stone", "flower", "goldSlimeBubble", "greenJellyCube", "icedCoffee", "woodFloor", "woodWall", "woodDoor", "torch"]'), "shared loot must accept all current building piece types");
...
assert(clientWorld.includes('treeTrunkImage.src = "assets/interactive_tree_trunk_damaged_v376.png?v=407";'), "fresh tree must use the clean swapped trunk state");
assert(clientWorld.includes('treeDamagedTrunkImage.src = "assets/interactive_tree_trunk_v376.png?v=407";'), "first-hit tree must use the cut swapped trunk state");
