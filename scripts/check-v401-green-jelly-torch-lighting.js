...
assert(server.includes('torch: Object.freeze({ resourceKey: "torches", outputCount: 1, ingredients: Object.freeze({ wood: 1, greenJellyCubes: 1 }) })'));
...
assert(server.includes('kind === "caveDoor" ? "stoneArches" : kind === "woodDoor" ? "woodDoors" : kind === "chest" ? "chests" : kind === "craftingTable" ? "craftingTables" : "torches"'));
assert(server.includes('spawnSharedResource(\n    removed.mapId,\n    removed.kind,'));
assert(server.includes('"greenJellyCube", "icedCoffee", "woodFloor", "stoneFloor", "woodWall", "stoneWall", "stoneCube", "caveDoor", "woodDoor", "torch", "chest", "craftingTable"'));
