...
assert(server.includes('message?.kind === "torch" ? "torch" : message?.kind === "chest" ? "chest" : null'));
assert(server.includes('kind === "woodDoor" ? "woodDoors" : kind === "chest" ? "chests" : "torches"'));
assert(server.includes('spawnSharedResource(\n    removed.mapId,\n    removed.kind,'));
assert(server.includes('"greenJellyCube", "icedCoffee", "woodFloor", "stoneFloor", "woodWall", "woodDoor", "torch", "chest"'));
