...
assert(server.includes('const BUILD_VERSION = "6-11-353";'), "server build must be v351");
assert(config.includes('const CLIENT_BUILD_VERSION = "6-11-353";'), "client build must be v351");
assert(pkg.version === "0.6.11.353", "package version must be v351");
assert(html.includes('id="mobileMenuButton"') && input.includes('handleMenuKeyDown("escape")'), "mobile MENU bridge missing");
assert(input.includes('MOBILE_COMBAT_ASSIST_MELEE_DISTANCE') && input.includes('MOBILE_COMBAT_ASSIST_BOW_DISTANCE'), "mobile combat assist ranges missing");
...
assert(html.includes('translateX(-50%) scale(.64)') && html.includes('env(safe-area-inset-bottom) + 24px'), "mobile toolbar/joystick tuning missing");
...
assert(html.includes('/client-input.js?v=353') && html.includes('/client-app.js?v=353'), "v351 client cache keys missing");
assert(adopted.version === 50, "live Drive map override revision 50 was not preserved");
