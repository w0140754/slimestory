...
assert(server.includes('const BUILD_VERSION = "6-11-471";'), "server build must be v355");
assert(config.includes('const CLIENT_BUILD_VERSION = "6-11-471";'), "client build must be v355");
assert.strictEqual(pkg.version, "0.6.11.471", "package version must be v355");
...
assert(combat.includes('function currentMeleeReach() {\n  return SWORD_REACH;\n}'), "standard melee reach must remain 26 after retired Katana removal");

assert(server.includes('const reach = 26;'), "authoritative enemy melee reach validation missing");
