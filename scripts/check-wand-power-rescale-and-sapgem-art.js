...
assert(server.includes('const BUILD_VERSION = "6-11-354";'), "server build must be 6-11-354");
assert(config.includes('const CLIENT_BUILD_VERSION = "6-11-354";'), "client build must be 6-11-354");
assert(pkg.version === "0.6.11.354", "package version must be 0.6.11.354");
assert(html.includes('/shared/combat-balance.js?v=354') && html.includes('/game.js?v=354'), "v336 cache keys missing");
assert(balanceText.includes('const VERSION = 27;'), "combat balance version must be 27");
assert(fs.existsSync(path.join(root, "public", "assets", "sapgem_wand_v3.png")), "redrawn Sapgem sprite missing");
assert(game.includes('sapgemWandImage = loadImage("assets/sapgem_wand_v3.png?v=347")'), "redrawn Sapgem sprite not wired with fresh cache key");
