...
assert(pkg.version === "0.6.11.336", "package version must be 0.6.11.336");
assert(html.includes('/shared/combat-balance.js?v=25') && html.includes('/game.js?v=336'), "v336 cache keys missing");
assert(balanceText.includes('const VERSION = 25;'), "combat balance version must be 25");
assert(fs.existsSync(path.join(root, "public", "assets", "sapgem_wand_v3.png")), "redrawn Sapgem sprite missing");
assert(game.includes('sapgemWandImage = loadImage("assets/sapgem_wand_v3.png?v=336")'), "redrawn Sapgem sprite not wired with fresh cache key");
...
assert(readme.startsWith('## v6-11-336 — Sapgem rotation fix'), "README v336 changelog missing");
